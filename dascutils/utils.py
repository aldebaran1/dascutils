...
def interpWeights(x, y, provisional_image: np.ndarray, d=2, N=512):
    xgrid, ygrid = np.meshgrid(np.linspace(np.nanmin(x), np.nanmax(x), N),
                               np.linspace(np.nanmin(y), np.nanmax(y), N))
...
def interpSpeedUp(x_in: Union[list,np.ndarray] = None,
    ...
    # resolution N
    xgrid, ygrid = np.meshgrid(np.linspace(np.nanmin(x_in), np.nanmax(x_in), N),
    ...
    # trinagulate to new grids, simplex wights for a new grid, barycentric 
    # coordinates based on the simplex for the new grids are computed
    if verbose: print ('Computing intepolation weights')
    t0 = datetime.datetime.now()
    vtx, wts = _interpWeights(xy, uv)
    # Interpolate N images
    Zim = np.copy(image) * np.nan
    if len(image.shape) > 2:
        if verbose: print ('Interpolating {} images'.format(image.shape[0]))
        for i in range(image.shape[0]):
            if verbose: print ('Image {}/{}'.format(i+1, image.shape[0]))
            Zim[i] = _interpolate(image[i], vtx, wts).reshape(N, N)
    else:
        Zim = _interpolate(image, vtx, wts).reshape(N, N)
    if verbose: print ('Interpolation done in {} seconds'.format((datetime.datetime.now() - t0).total_seconds()))
